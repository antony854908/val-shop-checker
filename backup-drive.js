...
// Helper to find first existing executable path
...
if (isWin) {
  ...
  const SEVEN_ZIP_EXE = findExecutable([
  ...
  const RCLONE_EXE = findExecutable([
    'C:/Users/thane/AppData/Local/Microsoft/WinGet/Packages/Rclone.Rclone_Microsoft.Winget.Source_8wekyb3d8bbwe/rclone-v1.75.1-windows-amd64/rclone.exe',
    'C:/Users/thane/AppData/Local/Microsoft/WinGet/Packages/Rclone.Rclone_Microsoft.Winget.Source_8wekyb3d8bbwe/rclone-v1.75.0-windows-amd64/rclone.exe',
    'rclone'
  ]);
  ...
  const excludeFlags = '-xr!node_modules -xr!.git -xr!.session-store.json* -xr!tmp-* -y';

  try {
    execSync(`"${SEVEN_ZIP_EXE}" a -t7z "${versioned7z}" "${PROJECT_DIR}" ${excludeFlags}`, { stdio: 'inherit' });
    console.log(`[OK] Created archive: ${versioned7z}`);
  } catch (err) {
  ...
  try {
    execSync(`"${RCLONE_EXE}" copy "${versioned7z}" "gdrive,root_folder_id=${GDRIVE_FOLDER_ID}:" --drive-acknowledge-abuse`, { stdio: 'inherit' });
    console.log('\n============================================================');
    ...
    try {
      execSync(`"${RCLONE_EXE}" copy "${versioned7z}" gdrive:VALORANT-Backups/ --drive-acknowledge-abuse`, { stdio: 'inherit' });
  ...
  const excludeFlags = '-xr!node_modules -xr!.git -xr!.session-store.json* -xr!tmp-* -xr!server.log -y';
  try {
    execSync(`7z a -t7z "${versioned7z}" "${PROJECT_DIR}" ${excludeFlags}`, { stdio: 'inherit' });
  ...
  let uploaded = false;
  try {
    execSync(`rclone copy "${versioned7z}" "gdrive,root_folder_id=${GDRIVE_FOLDER_ID}:" --drive-acknowledge-abuse`, { stdio: 'inherit' });
    console.log(`[SUCCESS] Uploaded ${sevenZipName} directly to Google Drive!`);
    uploaded = true;
  } catch (e) {
    try {
      execSync(`rclone copy "${versioned7z}" gdrive:VALORANT-Backups/ --drive-acknowledge-abuse`, { stdio: 'inherit' });
